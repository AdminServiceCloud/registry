...
- **CI** (`.github/workflows/ci.yml`) на каждый PR и push в main гоняет `npm run validate` ([scripts/validate.mjs](scripts/validate.mjs)): валидация всех файлов по схемам + структурные проверки — ссылки `index` существуют, нет файлов-сирот, имена пакетов уникальны во всём реестре, `category` соответствует позиции в иерархии, у `source` есть `git` или `url`, описания без кириллицы. Перед PR запускай локально.
